...
from src.agents.writer import (
    ...
    _add_figure_placeholders,
    _add_table_placeholders
)
...
def _remove_duplicate_headers(content: str) -> str:
    """
    Удаляет дублирующиеся заголовки ВВЕДЕНИЕ и ЗАКЛЮЧЕНИЕ.
    Оставляет только один заголовок первого уровня.
    """
    # Для ВВЕДЕНИЯ: убираем дубли типа "# ВВЕДЕНИЕ. ВВЕДЕНИЕ. Введение"
    intro_pattern = r'^(#\s*)?ВВЕДЕНИЕ[.\s]*(#\s*)?ВВЕДЕНИЕ[.\s]*(#\s*)?(Введение|ВВЕДЕНИЕ)?'
    content = re.sub(intro_pattern, '# ВВЕДЕНИЕ', content, flags=re.MULTILINE | re.IGNORECASE)
    
    # Для ЗАКЛЮЧЕНИЯ: убираем дубли типа "# ЗАКЛЮЧЕНИЕ. Заключение"
    conclusion_pattern = r'^(#\s*)?ЗАКЛЮЧЕНИЕ[.\s]*(#\s*)?(Заключение|ЗАКЛЮЧЕНИЕ)?'
    content = re.sub(conclusion_pattern, '# ЗАКЛЮЧЕНИЕ', content, flags=re.MULTILINE | re.IGNORECASE)
...
class DocumentGenerator:
    ...
    DOC_HEADER_TEMPLATES = {
        ...
        "vkr_report": "ВЫПУСКНАЯ КВАЛИФИКАЦИОННАЯ РАБОТА",
    }
    
    # Порядок разделов для ВКР: источники ПОСЛЕ заключения
    VKR_SECTION_ORDER = [
        "введен",           # Введение (строго первым)
        ...
        "6", "6.1", "6.2", "6.3", "6.4",
        "вывод по главе 6",
        "заключ",           # Заключение (строго перед источниками!)
        "источник",         # Список литературы (строго после заключения!)
        "приложен",         # Приложения (строго последними)
    ]
    ...
    def _vkr_sort_key(self, section_title: str) -> int:
        """Ключ сортировки для разделов ВКР."""
...
"""
        ...
        for section_title, content in sorted_sections:
            ...
            clean_content = _normalize_markdown_tables(clean_content)
    ...
    def _assemble_markdown_vkr(self, metadata: Dict, sections: Dict) -> str:
        """Сборка Markdown для ВКР с академическими метаданными"""
        yaml_header = f"""---
title: "{self._format_yaml_field(metadata.get('title', ''))}"
author: "{self._format_yaml_field(metadata.get('author', ''))}"
faculty: "{self._format_yaml_field(metadata.get('faculty', ''))}"
department: "{self._format_yaml_field(metadata.get('department', ''))}"
specialty: "{self._format_yaml_field(metadata.get('specialty', ''))}"
degree: "{self._format_yaml_field(metadata.get('degree', ''))}"
reviewer: "{self._format_yaml_field(metadata.get('reviewer', ''))}"
supervisor: "{self._format_yaml_field(metadata.get('supervisor', ''))}"
year: "{metadata.get('year', datetime.now().year)}"
bibliography: "{metadata.get('bibliography_file', 'references.bib')}"
...
"""
        ...
        # === Сортировка разделов ПО СТРОГОМУ ПОРЯДКУ ВКР ===
        sorted_sections = sorted(
            sections.items(),
            key=lambda x: self._vkr_sort_key(x[0])
        )
        
        for i, (section_title, content) in enumerate(sorted_sections):
            clean_content = content.strip()
            ...
            # Удаляем дубли ВВЕДЕНИЕ/ЗАКЛЮЧЕНИЕ
            clean_content = _remove_duplicate_headers(clean_content)
            
            # === ДОБАВЛЕНИЕ НУМЕРАЦИИ ФОРМУЛ ===
            clean_content = self._number_formulas(clean_content, section_title)
            
            # === ДОБАВЛЕНИЕ ПЛЕЙСХОЛДЕРОВ ДЛЯ РИСУНКОВ/ТАБЛИЦ ===
            clean_content = _add_figure_placeholders(clean_content, section_title)
            clean_content = _add_table_placeholders(clean_content)
            
            md += f"{clean_content}\n"
            
            # Добавляем разрыв страницы между разделами (кроме последнего)
            if i < len(sorted_sections) - 1:
                md += '\n<div style="page-break-after: always;"></div>\n\n'
        
        return md

    def _number_formulas(self, content: str, section_title: str) -> str:
        """
        Добавляет нумерацию к выключным формулам в формате $$...$$.
        Формулы в тексте ($...$) не нумеруются.
        """
        # Находим все выключные формулы $$...$$ без тега \tag{}
        pattern = r'\$\$(?!\s*\\tag\{)(.+?)\$\$'
        ...
        def add_tag(match):
            ...
            # Извлекаем номер главы из section_title (например, "2.3" → "2")
            chapter_num = re.match(r'^(\d+)', section_title)
            chapter = chapter_num.group(1) if chapter_num else "1"
            numbered = f"$${formula} \\tag{{{chapter}.{counter}}}$$"
            counter += 1
            return numbered
        
        return re.sub(pattern, add_tag, content, flags=re.DOTALL)
    ...
    def _remove_duplicate_goals_headers(content: str) -> str:
        ...
        for line in lines:
            ...
            if re.match(r'^#+\s*Цели(?:\s+раздела)?\s*$', stripped, re.IGNORECASE):
                goals_count += 1
                if goals_count == 1:
                    result.append(line)
            else:
    ...
    def generate_pdf(self,
        ...
        if doc_type == "vkr_report":
            # Подготовка метаданных для ВКР
            vkr_metadata = {
                "title": title,
                "author": organization,
                "reviewer": context.get("reviewer", ""),
                "institution": context.get("institution", "СибАДИ"),
                ...
                "bibliography_file": context.get("bibliography_file", "references.bib"),
            }
            full_md = self._assemble_markdown_vkr(
                metadata=vkr_metadata,
                sections=markdown_content
            )
        else:
            # Стандартная сборка для политик ИБ
